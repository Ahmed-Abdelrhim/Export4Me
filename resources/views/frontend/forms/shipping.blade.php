...
@section('content')
    <div class="row">
        <form action="{{route('shipping.company.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <input name="media" type="text" placeholder=" ادخل صفحه الميديا " class="box form-control @error('owner_name') is-invalid @enderror"
                   value="{{old('media')}}"/>
            @error('media')
            <span class="text-danger">{{$message}}</span>
            @enderror
...
{{--            <input name="landline" type="number" placeholder="ادخل رقم التليفون الأرضي" class="box form-control @error('owner_name') is-invalid @enderror"
value="{{old('landline')}}"/>--}}
            <input name="landline" type="number" placeholder="02xxxxxxxx" class="box form-control @error('landline') is-invalid @enderror" value="{{old('landline')}}"/>
            @error('landline')
            <span class="text-danger">{{$message}}</span>
            @enderror
...
{{--            <input name="phone_number" type="number" placeholder="ادخل رقم الهاتف المحمول" class="box form-control @error('owner_name') is-invalid @enderror"
id="phone_number" value="{{old('phone_number')}}"/>--}}
            <input name="phone_number" type="number" placeholder="01xxxxxxxxx" class="box form-control @error('phone_number') is-invalid @enderror" id="phone_number"
            ...
            <input name="shipping_types" type="text" placeholder="ادخل أنواع الشحن" class="box form-control @error('shipping_types') is-invalid @enderror"
                   value="{{old('shipping_types')}}" id="shipping_types"/>
